    ...
    <div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 items-center justify-items-center'>
      <div className='relative w-48 h-48 flex items-center justify-center col-span-1 md:col-span-1 lg:col-span-1'>
        <CircularProgressbar
          value={percentage}
          styles={buildStyles({
            pathColor: percentage < 51 ? '#34D399' : percentage < 76 ? '#FBBF24' : percentage < 91 ? '#FB923C' : '#EF4444',
            trailColor: '#374151',
            textSize: 10,
            textColor: percentage < 51 ? '#34D399' : percentage < 76 ? '#FBBF24' : percentage < 91 ? '#FB923C' : '#EF4444',
          })}
      ...
      <div className='flex flex-col items-center md:items-start gap-4 col-span-1 md:col-span-1 lg:col-span-2 text-center md:text-left'>
        <div className="text-xl text-gray-200 w-full">
          ...
          <DisplayAmount label='Disponible' amount={availableBudget} textColor={percentage < 51 ? 'text-emerald-400' : percentage < 76 ? 'text-yellow-400' : percentage < 91 ? 'text-orange-400' : 'text-red-500'} />
          <DisplayAmount label='Gastado' amount={totalSpent} textColor={percentage < 51 ? 'text-emerald-400' : percentage < 76 ? 'text-yellow-400' : percentage < 91 ? 'text-orange-400' : 'text-red-500'}/>          
        </div>
